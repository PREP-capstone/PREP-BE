...
> v1.7 변경: **비의료 건강관리서비스 2차본 `II.19` 오기 발견 및 보정 스크립트 추가.**
> 운영 RDS `correction_rules`에 `kr-mohw-nonmedical-health-guide-202209` +
> `legal_basis_article='II.19'`를 참조하는 3건(`진단 질환`, `처방 질환`, `치료 질환`)이
> 있었으나, RAG 2차본의 실제 section_id에는 `II.19`가 없다. 해당 표현의 근거는
> `II.3`(의료행위 판단 기준)이므로 `scripts/fix_nonmedical_section_id_ii19.py`로
> `II.19` → `II.3` 보정한다.
> v1.8 변경: **생명윤리 및 안전에 관한 법률(법률 제21065호) 유전자검사 핵심 조문 6건을
> RAG 메타데이터와 청크 초안에 추가했다.** `제2조제15호`/`제2조제16호`/`제46조`/`제49조`/
> `제49조의2`/`제50조`를 `kr-bioethics-safety-act-20251001`로 관리하며, 유전자 관련
> 룰베이스의 근거 조회 및 향후 quote 연결에 사용한다.
> v1.6 변경: **비의료 건강관리서비스 가이드라인 및 사례집 2차본(2022.9) 청킹 반영.**
...
| 비의료 건강관리서비스 가이드라인 및 사례집 | **2차(2022.9)** | `kr-mohw-nonmedical-health-guide-202209` | `kr-mohw-nonmedical-health-guide-202209` | ✅ **일치** | ✅ 장/절 + Q&A 단위(`I`, `II.3`, `III.3`, `Q11` 등 31건) | correction_rules **29건** | ✅ 해소 | 2026-09-07 | 2019년 1차본은 일반 참고용으로 유지하고, 룰베이스 근거 조회는 2022년 2차본 document_id만 사용. 청킹 스크립트에서 31개/핵심 section/핵심 문구/중복 없음 검증. 운영 RDS에서 `II.19` 오기 3건 발견 — `진단 질환`/`처방 질환`/`치료 질환`은 `II.3`으로 보정 필요 |
...
## 표 2-1: 룰베이스 section_id 오기 보정 이력

| 문서 | 잘못된 조문 | 올바른 조문 | 대상 규칙 | 처리 |
|---|---|---|---|---|
| 비의료 건강관리서비스 가이드라인 및 사례집(2차) | `II.19` | `II.3` | `진단 질환`, `처방 질환`, `치료 질환` | `scripts/fix_nonmedical_section_id_ii19.py --apply`로 운영 DB 보정 |

---

## 표 3: RAG 적재 문서 전체 목록 (38건, `RAG_데이터수집_최종보고서.md` 2026-07-29 기준 + 실데이터 재확인)

표 1은 **룰베이스가 실제로 인용하는 문서만** 다룬다. 아래는 RAG에 등록된 **전체 38개 문서**를
청킹 여부까지 실데이터(`evidence_documents_draft.csv`/`evidence_chunks_draft.csv`)로 재확인한
목록이다 — 표 1에 없는 30건은 현재 룰베이스가 인용하지 않지만, 향후 규제/개인정보/광고 축
...
|---|---|---|---|---|
| `kr-mfds-wellness-0091-03-20260212` | 의료기기와 개인용 건강관리제품(웰니스) 판단기준 | GUIDE/OFFICIAL_GUIDE | 30건 (조문 단위, 완료) | 표1 참조 |
...
| document_id | 제목 | 유형 | 청킹 상태 | 비고 |
|---|---|---|---|---|
| `kr-medical-act-20260407` | 의료법 | LAW/ACT | 3건 (조문 단위, 완료) | 표1 참조 |
...
| `kr-mohw-nonmedical-health-guide-202209` | 비의료 건강관리서비스 가이드라인 및 사례집(**2차**, 2022.9) | GUIDE/MOHW_GUIDE | 31건 (장/절 + Q&A 단위, 완료) | 표1 참조 — 룰베이스 인용 대상 |
...
| document_id | 제목 | 유형 | 청킹 상태 | 비고 |
|---|---|---|---|---|
| `kr-bioethics-safety-act-20251001` | 생명윤리 및 안전에 관한 법률 | LAW/ACT | 6건(`제2조제15호`/`제2조제16호`/`제46조`/`제49조`/`제49조의2`/`제50조`) | 법률 제21065호, 2025.10.1. 시행. 유전자검사·유전정보·DTC·질병 관련 검사 제한 및 허위·과대광고 근거 |

### 요약

- **등록된 문서 39건 중 실제 청킹 완료는 11건**(413 chunks) — 나머지는 metadata만 등록되고 본문 청킹 전 단계
- 청킹 완료 11건: 웰니스판단기준(30, 정상) · 비의료가이드 1차(35, 일반 참고용) · 비의료가이드 2차(31, 정상) · 개인정보보호법(149) · 시행령(133) · 별표7(18, 정상) · 약사법(5) · 의료기기법(2) · 의료기기법시행규칙(1) · 의료법(3, 정상) · 생명윤리법(6, 유전자검사 핵심 조문)
- 룰베이스가 인용하는 8개 문서 중 **정상은 5개**(웰니스판단기준·비의료가이드 2차·의료기기법·별표7·의료법, 약사법은 제44조만 미청킹) — 모바일앱지침·LLM가이드라인은 낮은 우선순위 확인 대상
...
- 모바일 의료용 앱 안전관리 지침과 LLM 기반 디지털의료기기 가이드라인을 RAG에 청킹할 때
- `judgement/*` quote 화이트리스트를 추가 변경할 때

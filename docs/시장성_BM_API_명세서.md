...
> category_1은 한때 문서상 7종(수면·정신건강·운동·식단·만성질환·여성건강·미용)으로
> 축소됐었으나(db_구축_설계서.md V3.3, 2026-07-20), 실제 배포된 분류 모델은 그
> 결정과 무관하게 계속 **유전자**를 포함한 8종을 써왔다(채린 님 확인, 2026-08-22).
> 2026-09-14 팀 재검토로 유전자 재포함이 최종 확정되어 문서도 8종으로 원복함
> (db_구축_설계서.md V4.6 동기화). DB 컬럼은 `String(80)` 자유 텍스트라 값 종류가
> 늘어도 스키마 변경은 필요 없다.
